...
int EnemigoDerecha(int x, int y, Ficha* player, Tablero* tab, Ficha* oponentes)
{
    ...
    if(player -> dir == 1)//si la direccion de nuestra ficha es hacia arriba
    {
        ...
        //Si no nos estorba ninguna ficha amiga u oponente para comer
        if((Comida == 0 || Comida == 2) && (AmigaL == 0 || AmigaL == 2))
        {
            if(AmigaC == 2)//si tenemos una amiga a la izquierda
            {
                //si seleccionamos una casilla valida para comer a la derecha
                if(x < player -> x + 245 && x > player -> x + 155 && y > player -> y - 245 && y < player -> y -155)
                {
                    //realizamos movimiento y nos comemos la ficha llamando a estas funciones
            ...
            if(AmigaC == 0)//si no tenemos amigas en ningun lado
            {
                //si seleccionamos una casilla valida para comer a la derecha
                if(x < player -> x + 245 && x > player -> x + 155 && y > player -> y - 245 && y < player -> y -155)
                {
                    //movemos la ficha y nos comemos la otra
                    MovBlancasConComida(player, 1,tab);
                    comerFNegraDerecha(player, oponentes, tab);
                    return 1;
                }
                //si presionamos la casilla a la izquierda porque esta disponible
                if(x > player -> x - 145 && x < player -> x - 55 && y > player -> y - 145 && y < player -> y - 55)
                {
                    //si nos encontramos en una de la penultima casilla de arriba o abajo en el tablero
                    if(player -> x == 650 || player -> y == 150)
                    {
                        //movemos la ficha a la izquierda sin comer
                        MovBlancasSinComida(player,0, tab);
                        return 1;
                    }else
                    {
                        //eliminamos la ficha si hubo oportunidad de comer a la derecha y no se hizo (por bobo)
                        eliminarf(player, tab);
                        return 1;
                    }
                }
        ...
        if((Comida == 1 || Comida == 0) && (AmigaL == 1 || AmigaL == 0))
        {
            ...
            if(AmigaC == 0)
            {
                ...
                if(x < player -> x + 145 && x > player -> x + 55 && y > player -> y -145 && y < player -> y - 55)
                {
                    ...
                    {
                        //si hibo oportunidad de comer y no se hizo, se pierde la ficha del jugador en turno por bobo.
                        eliminarf(player, tab);
                        return 1;
                    }
                }
